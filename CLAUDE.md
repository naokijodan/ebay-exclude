...
| `mcp__ebay-exclude__bulk_update_exclusions` | フィルタ付き一括除外更新 | 「全ポリシーからMiddle Eastを除外して」 |

### bulk_update_exclusions の引数（一括更新はこれを使う）
- `filter`: ポリシー名の部分一致フィルタ（省略で全ポリシー対象）
- `addExclusions`: 追加する除外項目の表示名配列
- `removeExclusions`: 解除する除外項目の表示名配列
- `dryRun`: デフォルトtrue（プレビューのみ）。実際に反映するには `false`
